...
export class PowerZoneManager extends PIXI.Container {
  ...
  constructor(addDot, removeDot, addMultipleDots, removeMultipleDots, rezoneDot, setDivisionResult, displayUserMessage, soundManager, wantedResult) {
    super();
    this.addDot = addDot;
  ...
  init(textures, spritePool, base, usage_mode, operator_mode, totalZoneCount, placeValueOn, negativePresent) {
    ...
    this.allZones = [];
    this.isInteractive = usage_mode === USAGE_MODE.FREEPLAY;
    this.pendingAction = [];
    this.explodeEmitter = [];
    this.implodeEmitter = [];
      ...
      if (droppedOnPowerZoneIndex != -1) {
        ...
        if (this.totalZoneCount - droppedOnPowerZoneIndex >= allZonesValue.length == false) {
          ...
          for (let i = 0; i < allZonesValue.length; i++) {
            ...
            } else if (this.allZones[droppedOnPowerZoneIndex + i] !== undefined) {
              this.allZones[droppedOnPowerZoneIndex + i].setBackgroundColor(PowerZone.BG_RED);
            }
          }
          if (isDragEnd === true) {
            ...
            if (success || antiSuccess) {
              ...
              if (dotsToMove.length > 0) {
                dotsToMove.forEach((dot) => {
                  ...
                  let finalPosition;
                  if (success) {
                    finalPosition = this.allZones[droppedOnPowerZoneIndex].toGlobal(this.allZones[droppedOnPowerZoneIndex].positiveDivideCounter.position);
                  } else if (antiSuccess) {
                    finalPosition = this.allZones[droppedOnPowerZoneIndex].toGlobal(this.allZones[droppedOnPowerZoneIndex].negativeDivideCounter.position);
                  }
                ...
                TweenMax.delayedCall(0.6, this.processDivisionAfterTween.bind(this), [dotsRemovedByZone, zoneOverInfo.actualZone, success]);
              }
            } else {
  ...
  processDivisionAfterTween(dotsRemovedByZone, moveToZone, isPositive) {
    ...
    if (isPositive) {
      this.setDivisionResult(moveToZone.zonePosition, moveToZone.positiveDivisionValue + 1, isPositive);
    } else {
      this.setDivisionResult(moveToZone.zonePosition, moveToZone.negativeDivisionValue + 1, isPositive);
    }
  }
  ...
  setDividerValueAfterBalance(zonePos, isPositive) {
    ...
    if (isPositive) {
      this.setDivisionResult(zonePos + 1, this.allZones[zonePos + 1].positiveDivisionValue + 1, isPositive);
    } else {
      this.setDivisionResult(zonePos + 1, this.allZones[zonePos + 1].negativeDivisionValue + 1, isPositive);
    }
        ...
        if (dotSprite.dot.isPositive === droppedOnPowerZone.isPositive) {
          ...
          if (dotSprite.dot.isPositive) {
            ...
            if (Object.keys(this.allZones[originalZoneIndex].negativeDots).length > 0) {
              ...
              const negativeSprite = this.allZones[originalZoneIndex].negativeDotsContainer.getChildAt(0);
            ...
            if (Object.keys(this.allZones[originalZoneIndex].positiveDots).length > 0) {
              const allRemovedDots = [];
              const positiveSprite = this.allZones[originalZoneIndex].positiveDotsContainer.getChildAt(0);
  ...
  getZoneUnderCursor(data) {
    ...
    const dataLocalZone = data.getLocalPosition(this.leftMostZone);
  ...
  tweenDotsToNewZone(originalZoneIndex, droppedOnPowerZone, dotsToRemove, positionToBeMovedTo, isPositive) {
    ...
    if (this.base[0] > 1) {
      ...
      for (let i = 0; i < dotsToRezone; i++) {
        ...
        const finalPosition = this.movingDotsContainer.toLocal(adjacentPosition, droppedOnPowerZone);
    ...
    const notDisplayedDots = this.allZones[originalZoneIndex].removeNotDisplayedDots(notDisplayedDotCount, isPositive);
  ...
  adjustDividerDotFromStateChange(positiveDividerDots, negativeDividerDots) {
  ...
  populateDivideResult(positiveDividerResult, negativeDividerResult) {
    ...
    this.allZones.forEach((zone) => {
      ...
      zone.setDivisionValue(positiveDividerResult[zone.zonePosition], negativeDividerResult[zone.zonePosition]);
    });
    positiveDots.reverse();
    negativeDots.reverse();
    this.dividerResult.update(positiveDividerResult.slice(0).reverse(), negativeDividerResult.slice(0).reverse(), positiveDots, negativeDots);
  }
