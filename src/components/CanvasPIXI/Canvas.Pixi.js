...
class CanvasPIXI extends Component {
  ...
  resize(event) {
    ...
    const ratio = Math.min(w / SETTINGS.GAME_WIDTH, h / (this.props.operator_mode === OPERATOR_MODE.DIVIDE ? SETTINGS.GAME_HEIGHT_DIVIDE : SETTINGS.GAME_HEIGHT));
      ...
      if (this.props.usage_mode !== USAGE_MODE.FREEPLAY) {
        ...
        switch (this.props.operator_mode) {
          ...
          case OPERATOR_MODE.ADD:
            ...
            if (this.props.base[1] !== BASE.BASE_X) {
              const operandAValue = this.calculateOperandRealValue(dotsPerZoneA);// dotsPerZoneA.reverse().join('').replace(/\b0+/g, '');
              const operandBValue = this.calculateOperandRealValue(dotsPerZoneB);// dotsPerZoneB.reverse().join('').replace(/\b0+/g, '');
              this.soundManager.playSound(SoundManager.GO_SUCCESS);
              this.props.startActivityDoneFunc(dotsPos, operandAValue, operandBValue);
            } else {
                            // remove last char in the operand if it's a - or a +
              const operandAValue = this.removeTrailingSign(this.props.operandA);
              const operandBValue = this.removeTrailingSign(this.props.operandB);
              this.soundManager.playSound(SoundManager.GO_SUCCESS);
              this.props.startActivityDoneFunc(dotsPos, operandAValue, operandBValue);
            }
            break;
